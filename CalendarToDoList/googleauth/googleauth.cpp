...
GoogleAuth::GoogleAuth(QObject* parent) : QObject(parent) {
  ...
  if (!data.exists()) QT_THROW("The file auth.json does not exists");
  ...
  google->setClientIdentifierSharedKey(
      settingsObject["client_secret"].toString());
  google->setAccessTokenUrl(QUrl(settingsObject["token_uri"].toString()));

  quint16 port =
      8001;  // static_cast<quint16>(QUrl(settingsObject["redirect_uris"].toArray()[0].toString()).port());
